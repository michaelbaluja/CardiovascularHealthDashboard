...
def get_hospital_data(zip: str):
    ...
    engine = SearchEngine()
    # lat_str = filtered_df['LATITUDE'].iloc[0]
    # lng_str = filtered_df['LONGITUDE'].iloc[0]
    ...
    fig = px.scatter_mapbox(hosp_df, lat="LATITUDE", lon="LONGITUDE", hover_name="NAME", hover_data=["ADDRESS", "WEBSITE"],
                            color_discrete_sequence=["light blue"], title="Map of Nearby Hospitals",
                            zoom=12, height=400, center=center_dict)
