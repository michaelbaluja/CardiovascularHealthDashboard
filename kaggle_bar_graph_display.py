...
data = pd.read_csv("kaggle_cleaned.csv")[['smoke', 'gender', 'alco', 'active', 'cardio']]
...
def changing_plot(cols):
    ...
    if len(cols) == 1:
        ...
        unique_combs = [j for j in itertools.product(unique_combs, data[cols[1]].unique().tolist())]
        if len(cols) > 2:
            for i in range(2, len(cols)):
                unique_combs = [j[0]+(j[1],) for j in itertools.product(unique_combs, data[cols[i]].unique().tolist())]
        for i in unique_combs:
            counts[i] = data.groupby(by=cols).mean()['cardio'].loc[i]
        output = pd.DataFrame(counts.values(), index= [f"{i}" for i in counts.keys()])
